...
train_dataloader = DataLoader(VaTeXDataset(VATEX_TRAIN_FILES), batch_size=BATCH_SIZE, collate_fn=pad_to_longest, shuffle=True)
val_dataloader = DataLoader(VaTeXDataset(VATEX_VAL_FILES, train=False), batch_size=BATCH_SIZE, collate_fn=pad_to_longest, shuffle=True) # remove shuffle

# Load model
model = TransformerModel(SOURCE_VOCAB_SIZE, TARGET_VOCAB_SIZE, d_model=512, N=NUMBER_OF_LAYERS, video_emb_size=VIDEO_EMB_SIZE).to(device)
...
for epoch in range(EPOCHS):
    ...
    # first = True
    # for vid, src, tgt in tqdm(train_dataloader, desc=f'Train epoch {epoch}'):
    for vid, src, tgt in train_dataloader:
        ...
        # if first:
        #   print(sp_en.decode(src[0].tolist()))
        #   print(sp_zh.decode(tgt[0].tolist()))
        #   first = False
    ...
    if (epoch+1) % VALIDATION_EPOCHS == 0:
        ...
        with torch.no_grad():
            ...
            #bleus = []
            #first = True
            # for vid, src, tgt in tqdm(val_dataloader, desc='Val set'):
            for vid, src, tgt in val_dataloader:
                ...
                #print(out)
                #for h, t in zip(out, tgt):
                #    t = sp_zh.decode(t.tolist())
                #    h = h[0]
                #    if first:
                #        print(t)
                #        print(h)
                ...
                #    #print(h)
                #    #break
                #    bleus.append(sentence_bleu(t, h))
                #hypos += [sp_zh.encode(o)[0] for o in out]
                #for row in tgt: refs.append([sp_zh.encode(sp_zh.decode(row.tolist()))])

                # val_loop.update(1)
                # val_loop.set_description(f'  Val Epoch {epoch}, loss: {loss:.4f}')
            
            #bleu = sum(bleus)/len(bleus)
            #print('BLEU Score', bleu, flush=True)
