...
from dataset import VaTeXCLIPDataset, MADDataset, OpusDataset
...
if source_model != 'false': # true / false
    source_model_path = f'../../compute/models/vatex-clip/{model_type}/{prefix}{source_model}_only'
else:
    source_model_path = ''
    source_model = ''
lr = float(lr)
...
if dataset == 'mad':
    ...
    train_dataset = VaTeXCLIPDataset([f'{prefix}clip_vatex_train.json'], tokenizer)
    val_dataset = VaTeXCLIPDataset([f'{prefix}clip_vatex_validation.json'], tokenizer)
else:
...
save_path = f'../../compute/models/vatex-clip/{model_type}/{prefix}{dataset}_{model_suffix}'
